/**
 * Solo CRM's own mark and wordmark (T-260829-06), lifted out of Rail.tsx so
 * the Branding card's preview (T-260829-07) draws the *same* default the rail
 * draws rather than a second copy of it. A preview that has drifted from the
 * thing it previews is worse than no preview: the operator is shown what they
 * already have, and the one job of those two rows is to answer "what is there
 * now".
 *
 * Both are inline SVG on tokens — the rule in .claude/rules/ui-design.md, and
 * nothing under assets/ is reachable from the bundle. The source assets' hex
 * literals became the tokens they equal on the way in; tokens.css stays the
 * only place a colour is spelled out.
 *
 * ## Both are decorative, and neither carries the name any more
 *
 * The wordmark used to be `role="img" aria-label="Solo CRM"`. It cannot be now
 * that an operator can replace it: the brand block would keep announcing a
 * product name that is no longer what is drawn (T-260829-07's Scope). The
 * accessible name moved up to the block containing both images, where it is
 * derived from `workspace.name`; everything inside that block — built-in SVG
 * or an operator's own `<img alt="">` — is decorative markup carrying no text.
 */

/**
 * The 25 × 25 mark. `gradientId` is a required prop rather than the hard-coded
 * literal it used to be, because the rail and the settings preview are mounted
 * at the same time: two elements sharing one `id` is a duplicate id in the
 * document, and `url(#…)` resolves to whichever of them came first. Each call
 * site names its own.
 */
export function SoloCrmMark({ gradientId }: { gradientId: string }) {
  return (
    <svg viewBox="0 0 112 112" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
      ...
      <g transform="translate(56 56) scale(0.9167) translate(-48 -48)">
        <circle cx={48} cy={48} r={42} fill={`url(#${gradientId})`} />
...
/**
 * The wordmark: the lockup's glyphs alone, on a viewBox tightened to their
 * bounding box (109.5 39.5 183 33). solocrm-logo.svg itself is a 322 × 112
 * lockup whose glyphs occupy a corner of it; dropped into the 104px slot as-is
 * it would render mostly empty space, and therefore tiny. "Solo" carries no
 * fill so it inherits `currentColor` from `.wordmark` (--papyrus); "CRM" is
 * the accent and names its token.
 *
 * The caller supplies the class because the same glyphs render in the rail and
 * in the Branding card's preview, and both want `.wordmark`'s fixed box.
 */
export function SoloCrmWordmark({ className }: { className?: string }) {
  return (
    <svg
      className={className}
