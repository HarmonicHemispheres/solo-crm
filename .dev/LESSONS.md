...
10. System Node 22.12 segfaults better-sqlite3 under vitest. Use 22.22 or later.
11. A scope written from an assumption about a tool, not from running it, has been wrong every time it was checked. Run the tool while scoping.
12. Two changes each green alone can fail together. The full suite runs once, on the final tree, before close.
13. A UTC-midnight date compared against a local clock is off by a day west of UTC from about 17:00.
14. A partial update that does not name a column can silently NULL it. Diff against what the form was seeded with, not the record.
